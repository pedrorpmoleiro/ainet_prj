...
Route::middleware(['auth', 'verified', 'isNotDeleted'])->group(
    function () {
        ...
        Route::middleware(['isAtivo', 'isPasswordInicial'])->group(
            function () {
                Route::resource('socios', 'UserController')->only(['index']);

                Route::middleware(['isDirecao'])->group(
                    function () {
                ...
                Route::resource('movimentos', 'MovimentoController')->only(['create', 'store'])
                    ->middleware(['can:canCreate,App\Movimento']);
                Route::resource('movimentos', 'MovimentoController')->only(['edit','update'])
                    ->middleware(['can:canUpdate,movimento']);
                Route::resource('movimentos', 'MovimentoController')->only(['destroy'])
                    ->middleware(['can:canDestroy,movimento']);
                ...
                Route::middleware(['isDirecao'])->group(
                    function () {
                        ...
                        Route::post('/aeronaves/{aeronave}/pilotos/{piloto}', 'AeronaveController@addPiloto')
                            ->name('aeronaves.add_piloto');
                        Route::delete('/aeronaves/{aeronave}/pilotos/{piloto}', 'AeronaveController@removePiloto')
                            ->name('aeronaves.remove_piloto');
